...
def startNetwork():
    ...
    info('** Test reachability before updates\n')
    # networkReachableTest()
    
    info('** Link Failure Trigger\n')
    tearDownLinkTest()
    
    info('** Test reachability after updates\n')
    # networkReachableTest()
...
def tearDownLinkTest():
    if net is not None:
        info('** Tear down link between s3-s8\n')
        net.configLinkStatus('s3', 's8', 'down')
        net.configLinkStatus('s8', 's3', 'down') 
